...
// Funzione per caricare i contenuti nella home page tramite API REST
async function loadContenuti() {
  try {
    const response = await fetch('api.php');
    const contenuti = await response.json();

    const contenutiSection = document.getElementById('contenuti');
    contenutiSection.innerHTML = '';

    contenuti.forEach(contenuto => {
      const contenutiItem = document.createElement('div');
      contenutiItem.innerHTML = `
        <h3>${contenuto.titolo}</h3>
        <p>${contenuto.descrizione}</p>
      `;
      contenutiSection.appendChild(contenutiItem);
    });
  } catch (error) {
...
// Carica i contenuti nella home page quando la pagina viene caricata
document.addEventListener('DOMContentLoaded', loadContenuti);

// Funzione per gestire il "like" di un elemento
async function likeElement(elementId) {
  try {
    const response = await fetch('api.php', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ action: 'like', elementId: elementId })
    });

    if (response.ok) {
      console.log('Like registrato con successo');
    } else {
      console.error('Errore durante il like dell\'elemento');
    }
  } catch (error) {
    console.error('Errore durante il like dell\'elemento:', error);
  }
}

// Funzione per gestire l'inserimento di un commento
async function addComment(elementId, comment) {
  try {
    const response = await fetch('api.php', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ action: 'comment', elementId: elementId, comment: comment })
    });

    if (response.ok) {
      console.log('Commento inserito con successo');
    } else {
      console.error('Errore durante l\'inserimento del commento');
    }
  } catch (error) {
